...
def normalize_awb(value):
    """
    Normaliza os formatos reais usados nas fontes:
    - AWBStatus: 35250460 ou números menores que precisam de zero à esquerda
    - Eu Entrego: 57735250460 -> remove o prefixo 577
    - Planilha da Torre / bipagem: 577352504600001 -> remove 577 e os 4 finais
    """
    if pd.isna(value):
        return None

    raw = str(value).strip()
    # Corrige números lidos do Excel como 12345678.0
    raw = re.sub(r"\.0$", "", raw)
    ...
    # Código completo de volume: 577 + AWB(8) + 4 dígitos
    if len(digits) == 15 and digits.startswith("577"):
        return digits[3:11]

    # Pedido do Eu Entrego: 577 + AWB(8)
    ...
    match = re.search(r"577(\d{8})", digits)
    if match:
